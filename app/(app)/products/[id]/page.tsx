...
async function getVariantCosts(variantId: number) {
  ...
  const costs = await db
    ...
    .innerJoin(
      formulation,
      sql`${formulation.id} = ${formulationIngredient.formulationId}`
    )
    .innerJoin(
      ingredient,
      sql`${ingredient.id} = ${formulationIngredient.ingredientId}`
    )
    .where(
      sql`${formulation.productVariantId} = ${variantId} AND ${formulation.isActive} = true`
    )
...
export default async function ProductPage({ params }: Props) {
  ...
  const averageCost =
    variantCosts.length > 0 ? totalCosts / variantCosts.length : 0;
